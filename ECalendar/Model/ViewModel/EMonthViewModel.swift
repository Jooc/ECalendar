...
extension EMonthViewModel{
    func generateMonth(baseWeekday: WeekdaySymbol, preCount: Int, curCount: Int) {
        ...
        for i in 0..<baseWeekday.rawValue{
            firstweek.days.append(EDayViewModel(model: EDay(date: preCount-baseWeekday.rawValue + 1 + i), state: .uncover))
        }
        count = 1
        while firstweek.days.count < 7{
            firstweek.days.append(EDayViewModel(model: EDay(date: count), state: .available))
            count += 1
        }
        ...
        while !finish{
            ...
            for _ in 1...7{
                week.days.append(EDayViewModel(model: EDay(date: count), state: .available))
                count += 1
                if count == curCount + 1{
                    count = 1
                    while week.days.count < 7{
                        week.days.append(EDayViewModel(model: EDay(date: count), state: .uncover))
                        count += 1
                    }
                    finish = true
                }
